...
class FileInputImpl: public InputImplBase {
 public:
  virtual bool Open(const std::string &filename, bool binary) {
    ...
    NSArray *parts = [NSArray arrayWithObjects: bundlePath, [NSString stringWithUTF8String: filename.c_str()], nil];
    NSString *path = [NSString pathWithComponents:parts];
    const char *cpath = [path fileSystemRepresentation];
    std::string iosFile(cpath);

//    NSLog(@"Opening file %s, size %llu", filename.c_str(), [[NSFileManager defaultManager] attributesOfItemAtPath:path error:nil].fileSize);

    
    is_.open(cpath, binary ? std::ios_base::in|std::ios_base::binary
             : std::ios_base::in);
    return is_.is_open();
  }
...
class PipeInputImpl: public InputImplBase {
 public:
  ...
  virtual bool Open(const std::string &rxfilename, bool binary) {
    ...
    NSLog(@"Opening pipe/file for reading %s", filename_.c_str());
    KALDI_ASSERT(f_ == NULL);  // Make sure closed.
...
//  NSLog(@"Opening input stream for %s", rxfilename.c_str());
